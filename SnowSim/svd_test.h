...
__host__ __device__ static void jacobiConjugation(int x, int y, int z, mat3 &S, quat &qV)
{
 ...
 */
__host__ __device__ __forceinline__ static void jacobiEigenanalysis(mat3 &S, quat &qV)
{
	qV = quat(1, 0, 0, 0);
...
__host__ __device__ __forceinline__ static void sortSingularValues(mat3 &B, mat3 &V)
{
	// used in step 2
...
__host__ __device__ __forceinline__ static void QRGivensQuaternion_(float a1, float a2, float &ch, float &sh)
{
...
__host__ __device__ static void QRDecomposition(const mat3 &B, mat3 &Q, mat3 &R)
{
	R = B;
	...
	QRGivensQuaternion_(R[0], R[1], ch, sh);
	...
	QRGivensQuaternion_(R[0], R[2], ch, sh);
	...
	QRGivensQuaternion_(R[4], R[5], ch, sh);
 ...
 */
__host__ __device__ __forceinline__ static void computeSVD(const mat3 &A, mat3 &W, mat3 &S, mat3 &V)
{
 ...
 */
__host__ __device__ static void computePD(const mat3 &A, mat3 &R)
{
 ...
 * Can get Polar Decomposition from SVD, see first section of http://en.wikipedia.org/wiki/Polar_decomposition
 */
__host__ __device__ static void computePD(const mat3 &A, mat3 &R, mat3 &P)
{
	// U is unitary matrix (i.e. orthogonal/orthonormal)
	// P is positive semidefinite Hermitian matrix
	mat3 W, S, V;
 ...
 * PD : A = R * E
 */
__host__ __device__ static void computeSVDandPD(const mat3 &A, mat3 &W, mat3 &S, mat3 &V, mat3 &R)
{
	computeSVD(A, W, S, V);
	R = mat3::multiplyABt(W, V);
}
